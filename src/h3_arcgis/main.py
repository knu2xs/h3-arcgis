...
def _get_h3_ids(geojson_feat: dict, hex_level: int) -> int:
...
def get_unique_h3_ids_for_aoi(orig_df: pd.DataFrame, hex_level: int = 9) -> list:
    ...
    """
    # ensure the dataframe is a valid spatially enabled dataframe
    assert orig_df.spatial.validate(), 'The input Spatially Enabled Dataframe does not appear to be valid.'
    ...
    h3_id_lst = [_get_h3_ids(feat, hex_level) for feat in geojson_feat_lst]

    # combine all the id's and use a set to eliminate any duplicates
    h3_ids = set(chain.from_iterable(h3_id_lst))

    assert len(h3_ids) > 0, f'The resolution provided, H3 level {hex_level}, is too coarse to return any results. ' \
                            f'Please select a finer level of detail, say level {hex_level + 1}, and see if that ' \
                            f'works any better.'

    return list(h3_ids)


def get_esri_geometry_for_h3_id(h3_id: int) -> Polygon:
    """
    Convert an Uber H3 id to an ArcGIS Python API Polygon Geometry object.

    :param h3_id: Integer Uber H3 id.
...
def add_h3_ids_to_points(df: pd.DataFrame, h3_max: int, h3_min: int) -> pd.DataFrame:
    ...
    :param h3_max: Integer maximum H3 grid level defining the samllest geographic hex area - must be larger than the
        minimum.
    :param h3_min: Integer minimum H3 grid level defining the largest geograhpic hex area - must be smaller than the
        maximum.
...
def get_h3_ids_by_count(df: pd.DataFrame, min_count: int, count_column: str = None) -> pd.DataFrame:
    """
    Assign a H3 id to each point based on the H3 count in increasingly resolution H3 hexagons. If not enough present,
    the points for the summary tesselation cell will not be retained.
...
def get_h3_hexbins_with_counts(df: pd.DataFrame, h3_id_col: str = 'h3_id',
    ...
    :param h3_id_col: Column containing the Uber H3 id for each point.
...
def get_threshold_h3_hexbins(spatially_enabled_dataframe: pd.DataFrame, h3_minimum: int = 5,
    ...
    :param h3_maximum: Integer maximum H3 grid level defining the smallest geographic hex area - must be larger than the
        minimum.
    :param h3_minimum: Integer minimum H3 grid level defining the largest geographic hex area - must be smaller than the
        maximum.
